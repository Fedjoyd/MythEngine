...
// Operateurs
bool        Vec3::operator==  (Vec3 const& p_vector)   const
{
...
Vec3& Vec3::operator=   (Vec3 const& p_other)
{
    m_x = p_other.m_x;
    m_y = p_other.m_y;
    m_z = p_other.m_z;
...
float       Vec3::operator[]  (unsigned int const    p_indem_x)    const
{
    if (p_indem_x < 0 || p_indem_x > 2)
    {
        throw std::out_of_range("Indem_x is out of range, should be between 0 and 2");
    }
    return (p_indem_x == 2) ? m_z : (p_indem_x == 1) ? m_y : m_x;
}
float&      Vec3::operator[]  (unsigned int const    p_indem_x)
{
    if(p_indem_x < 0 || p_indem_x > 2)
	{
		throw std::out_of_range("Indem_x is out of range, should be between 0 and 2");
...
Vec3     Vec3::operator+   (Vec3 const& p_vector)   const
{
    return {m_x+p_vector.m_x, m_y+p_vector.m_y, m_z+p_vector.m_z};
}
Vec3     Vec3::operator+   (float const    p_value)    const
{
    return { m_x + p_value, m_y + p_value, m_z + p_value };
}
Vec3&    Vec3::operator+=  (Vec3 const& p_vector)
{
    m_x += p_vector.m_x;
    m_y += p_vector.m_y;
    m_z += p_vector.m_z;
...
const Vec3&    Vec3::operator+=  (Vec3& p_vector)
{
    m_x += p_vector.m_x;
    m_y += p_vector.m_y;
    m_z += p_vector.m_z;
...
Vec3&    Vec3::operator+=  (float const    p_value)
{
    m_x += p_value;
    m_y += p_value;
    m_z += p_value;
...
Vec3     Vec3::operator*   (float const    p_scale)    const
{
...
Vec3& Vec3::operator*=  (Vec3& p_vector) 
{
    m_x *= p_vector.m_x;
    m_y *= p_vector.m_y;
    m_z *= p_vector.m_z;
...
Vec3&    Vec3::operator*=  (float const    p_scale)
{
    m_x *= p_scale;
    m_y *= p_scale;
    m_z *= p_scale;
